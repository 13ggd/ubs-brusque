...
A button-tap digital version of the reception tally (each tap posting to a Google Form so counts land in
a spreadsheet without manual transcription) was built and then removed at the project's request — the
paper sheet is the method. If it ever comes back, the reason it was worth building is in this repo's git
history, and so is the reason paper won: the sheet survives a power outage and a dead network, two of the
very conditions its own Observação column asks staff to record.
